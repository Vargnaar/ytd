...
parser.add_argument('Url', metavar='url', type=str, nargs='?', default=None, help='the URL of the YouTube video')
parser.add_argument('-a', '--audio', action='store_true', help='download audio only')
parser.add_argument('-n', '--name', type=str, help='rename the downloaded file')
...
def download_video():
    ...
    while True:
        link = args.Url if args.Url else input("Enter link: ")
        ...
        try:
            ...
            if args.Url:
                download_option = '2' if args.audio else '1'
                folder = video_folder if download_option == '1' else audio_folder
                handle_download(yt, folder, download_option, args.name)
            else:
